...
', ]); } // Show error if user does not tick privacy on my account page add_filter("woocommerce_registration_errors", "validate_privacy_registration", 10, 3); function validate_privacy_registration($errors, $username, $email) { if
(!is_checkout()) { if (!(int) isset($_POST["privacy_policy_reg"]) || !(int) isset($_POST["terms_reg"])) { $errors->add( "privacy_policy_reg_error", __("Zaakceptuj politykę prywatności i regulamin, aby przejść dalej", "woocommerce"), ); } }
return $errors; }
